...
def process_simulation_ps(simfile_spec):
    ...
    for t in range (2, timing_int.shape[0]-22, 1):
        ...
        for i in range (0, ez.shape[1], 1):
            for j in range (0, ez.shape[2], 1):
                ez_ts = ez_int[t:t+21,i,j]*hanning_window
    ...
    # saving simulations
    np.save('./processing_results/' + simfile_sh + '.pshn_ps.npy', ps_total)
    np.save('./processing_results/' + simfile_sh + '.pshn_freqs.npy', freqs[idx])
    np.save('./processing_results/' + simfile_sh + '.pshn_inttiming.npy', timing_int[12:-12])
...
with mp.Pool(28) as p:
    p.map(process_simulation_ps, simfiles_spec_proc)
