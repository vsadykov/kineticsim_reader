...
def compute_powerspectrum(ez_ts):
...
for simfile_spec in simfiles_spec:
    ...
    for t in range (2, timing_int.shape[0]-22, 1):
        print("Current time step is " + str(t-2) + " from the total steps of " + str(timing_int.shape[0]-24))
        for i in range (0, ez.shape[1], 1):
            for j in range (0, ez.shape[2], 1):
    ...
    # saving simulations
    np.save(savefiles_folder + simfile_sh + '.ps_ps.npy', ps_total)
    np.save(savefiles_folder + simfile_sh + '.ps_freqs.npy', freqs[idx])
    np.save(savefiles_folder + simfile_sh + '.ps_inttiming.npy', timing_int[12:-12])
