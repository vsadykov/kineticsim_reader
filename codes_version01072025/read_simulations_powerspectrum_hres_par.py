...
def process_simulation_ps(simfile_spec):
    ...
    timing_nonint = tim[:,1]
    timing_int = np.arange(timing_nonint[0]-30.0, timing_nonint[-1]+30.0, 1.25)
    ...
    for t in range (2, timing_int.shape[0]-22, 1):
        ...
        for i in range (0, ez.shape[1], 1):
            for j in range (0, ez.shape[2], 1):
                ...
                freqs = np.fft.fftfreq(ps.size, 1.25)
                idx = np.argsort(freqs)
                ps_total[:,t-2] += ps[idx]
    ps_total /= ez.shape[1]*ez.shape[2]    
    # saving simulations
    np.save('./processing_results/' + simfile_sh + '.pshr_ps.npy', ps_total)
    np.save('./processing_results/' + simfile_sh + '.pshr_freqs.npy', freqs[idx])
    np.save('./processing_results/' + simfile_sh + '.pshr_inttiming.npy', timing_int[12:-12])
